...
export default function TeacherSidebar({ isOpen, setIsOpen, isCollapsed, setIsCollapsed }: TeacherSidebarProps) {
  const pathname = usePathname();

  const menuItems = [
    { name: 'Dashboard Guru', path: '/teacher/dashboard', icon: 'dashboard' },
    { name: 'Kelas Saya', path: '/teacher/courses', icon: 'menu_book' },
    { name: 'Evaluasi & Kuis', path: '/teacher/quizzes', icon: 'quiz' },
  ];
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Label — tidak dirender saat collapsed */}
                {!isCollapsed && (
